...
function RepeatedCriterion:updateOutput(input, target)
   self.output = 0
   if self.weights == nil  then self.weights = torch.ones(#input):div(#input) end 
   for i = 1,#input do
      self.output = self.output + self.weights[i]*self.criterion:updateOutput(input[i],target)
   end
   return self.output
end
...
function RepeatedCriterion:updateGradInput(input, target)
   ...
   for i = 1,#input do
      nn.utils.recursiveAdd(self.gradInput[i], self.weights[i], self.criterion:updateGradInput(input[i], target))
   end
