...
function ChainSPEN:features_net()
	...
	local f1 = nn.TemporalConvolution(self.feature_size,self.config.feature_hid_size,3,1)(x_pad)
...
function ChainSPEN:unary_energy_net()
	...
	local per_label_score = nn.TemporalConvolution(self.config.feature_hid_size,self.domain_size,1,1)(conditioning_values)
...
function ChainSPEN:global_energy_net()
	...
	local pairwise_potentials = nn.SoftPlus()(nn.TemporalConvolution(input_dim_to_joint_energy,energy_hid_size,2,1)(input_to_joint_energy))
	pairwise_potentials = nn.SoftPlus()(nn.TemporalConvolution(energy_hid_size,energy_hid_size,1,1)(pairwise_potentials))
	pairwise_potentials = nn.Sum(2)(nn.Reshape(-1)(nn.TemporalConvolution(energy_hid_size,1,1,1)(pairwise_potentials)))
