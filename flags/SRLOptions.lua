...
--SPEN Options
cmd:option('-use_official_scoring_script',0,'whether to additionally use the official scoring script (requires large max_predicates and max_arguments so that nothing gets thrown away)')
...
cmd:option('-feature_dim',200,'dimensionality of input features')
cmd:option('-hard_constraint_weight',3,'extra weight to place on the constraint that arguments with the same predicate do not overlap')
